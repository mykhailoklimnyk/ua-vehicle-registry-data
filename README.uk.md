...
> [!IMPORTANT]
> ### Джерело перестало публікувати номерні знаки і КОАТУУ власника
>
> Зі знімка **2026-05** Міністерство внутрішніх справ більше не подає у файлі ні реєстраційний номер, ні код КОАТУУ адреси власника. Це зміна на боці джерела — цей проєкт нічого не вилучав.
>
> Через це чотири колонки **відсутні у файлах за 2026-05 і пізніше**: `reg_addr_koatuu`, `n_reg_new`, `n_reg_latin`, `is_valid_plate`.
>
> Їх саме вилучено, а не залито порожнечею, щоб порожню колонку не сплутали з «немає значення». **Файли до 2026-04 не змінилися**, і релізи, опубліковані до переходу, і далі містять номери та КОАТУУ.
>
> **Що це означає на практиці**
> - Простежити автомобіль за номерним знаком між періодами після 2026-04 більше не вийде. `vin` лишається доступним — саме він тепер ідентифікатор.
> - Географічний аналіз за адресою реєстрації власника для нових періодів неможливий. Єдина просторова ознака, що лишилась, — сервісний центр (`dep_name`, `dep_code`).
> - Перевірити формат номера ніяк, тому `is_valid_plate` не подається — саме відсутній, а не `false`: валідність невідома, а не хибна.
>
> Тією ж зміною з'явилась нова колонка `power_kwt` (потужність двигуна в кВт); на давніші реєстрації того самого VIN її рознесено.
>
> **Якщо порівнюєте періоди, зважайте на зміну обсягу.** Місячна кількість записів за 2026-05…08 приблизно на **17% нижча** за ті самі місяці 2025-го, тоді як 2026-01…04 були приблизно на **8% вищі**. Ми не приписуємо це одній причині — порівняння через межу переходу варто робити обережно.
>
> Подробиці: [`schema/schema.md`](schema/schema.md).
...
### Схема 2.0 — несумісна зміна зі знімка 2026-04

Джерело змінило те, що публікує, — похідний набір змінюється разом із ним. Повне визначення — у [`schema/schema.md`](schema/schema.md).

| З | Зміна |
|---|---|
| 2026-04 | `record_ids` (масив) замінено на `record_id` (один ID запису джерела) |
| 2026-04 | додано `power_kwt` — потужність двигуна в кВт |
| 2026-05 | `reg_addr_koatuu`, `n_reg_new`, `n_reg_latin`, `is_valid_plate` **більше відсутні**: реєстр перестав публікувати номерний знак і КОАТУУ власника |

Чотири прибрані колонки вилучено з файлу, а не залито порожнечею: за порожньою колонкою не відрізнити «немає даних» від «немає значення». У знімках до 2026-04 вони лишаються, тож **раніше опубліковані релізи не змінюються**.

У знімках до 2026-03 колонок 27, у 2026-04 — 28, з 2026-05 — 24. Звертайтеся до колонок за назвою і вважайте кожну необов'язковою.

## Завантаження

Посилання **CSV**, **Parquet** і **DQ** у таблицях нижче завжди віддають поточну версію відповідного періоду.

Посилання **Release** відкриває реліз на GitHub, а прикріплені до нього файли — заморожений знімок того, що опублікували на ту дату. Якщо період відтоді перезбирали, у вкладеннях лишається давніший витяг, тоді як за посиланнями вище — виправлений. Релізи свідомо не чіпаємо, щоб DOI і далі вказував саме на ті байти, для яких його видано. **Для аналізу беріть посилання CSV/Parquet; реліз цитуйте тоді, коли потрібен незмінний об'єкт із посиланням.**
