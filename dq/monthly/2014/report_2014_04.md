# DQ Report: mvs_opendata — 2014-04

**Status:** PASS  
**Rows:** 106,511  
**Checks:** 12/16 passed, 4 failed  
**Timestamp:** 2026-03-19T13:56:02.087862

## Failed Checks

| Field | Check | Severity | Total | Failed | Detail |
|-------|-------|----------|------:|-------:|--------|
| capacity | range | info | 100349 | 3 | [0, 20000] |
| own_weight | range | info | 105342 | 289 | [40, 45000] |
| total_weight | range | info | 106373 | 39 | [60, 90000] |
| payload | range | info | 105332 | 27 | [0, 35000] |

## All Checks

| Field | Check | Status | Detail |
|-------|-------|--------|--------|
| record_ids | not_null | PASS | 100.00% |
| person_type | not_null | PASS | 100.00% |
| person_type | allowed_values | PASS | ['J', 'P'] |
| reg_addr_koatuu | distinct_count | INFO | 9806 |
| oper_code | distinct_count | INFO | 79 |
| oper_name | distinct_count | INFO | 93 |
| d_reg | not_null | PASS | 100.00% |
| d_reg | distinct_count | INFO | 29 |
| dep_code | not_null | PASS | 100.00% |
| dep_code | distinct_count | INFO | 277 |
| dep_name | distinct_count | INFO | 307 |
| brand | distinct_count | INFO | 798 |
| model | distinct_count | INFO | 3814 |
| vin | distinct_count | INFO | 0 |
| make_year | not_null | PASS | 100.00% |
| make_year | range | PASS | [1900, 2027] |
| make_year | distinct_count | INFO | 72 |
| color | not_null | PASS | 100.00% |
| color | distinct_count | INFO | 11 |
| kind | not_null | PASS | 100.00% |
| kind | distinct_count | INFO | 10 |
| body | distinct_count | INFO | 46 |
| purpose | not_null | PASS | 100.00% |
| purpose | distinct_count | INFO | 3 |
| fuel | distinct_count | INFO | 4 |
| capacity | range | FAIL | [0, 20000] |
| capacity | distinct_count | INFO | 1481 |
| own_weight | range | FAIL | [40, 45000] |
| own_weight | distinct_count | INFO | 3427 |
| total_weight | range | FAIL | [60, 90000] |
| total_weight | distinct_count | INFO | 2405 |
| n_reg_new | distinct_count | INFO | 102413 |
| payload | range | FAIL | [0, 35000] |
| payload | distinct_count | INFO | 3792 |
| secondary_fuel | distinct_count | INFO | 2 |
| n_reg_latin | distinct_count | INFO | 102372 |
| is_valid_plate | not_null | PASS | 100.00% |
| raw_vin | distinct_count | INFO | 0 |
| _table_ | row_count | PASS |  |