# DQ Report: mvs_opendata — 2013-04

**Status:** PASS  
**Rows:** 188,833  
**Checks:** 12/16 passed, 4 failed  
**Timestamp:** 2026-03-19T12:53:12.389586

## Failed Checks

| Field | Check | Severity | Total | Failed | Detail |
|-------|-------|----------|------:|-------:|--------|
| capacity | range | info | 180972 | 3 | [0, 20000] |
| own_weight | range | info | 182421 | 33 | [40, 45000] |
| total_weight | range | info | 187650 | 11 | [60, 90000] |
| payload | range | info | 182241 | 112 | [0, 35000] |

## All Checks

| Field | Check | Status | Detail |
|-------|-------|--------|--------|
| record_ids | not_null | PASS | 100.00% |
| person_type | not_null | PASS | 100.00% |
| person_type | allowed_values | PASS | ['J', 'P'] |
| reg_addr_koatuu | distinct_count | INFO | 12038 |
| oper_code | distinct_count | INFO | 84 |
| oper_name | distinct_count | INFO | 97 |
| d_reg | not_null | PASS | 100.00% |
| d_reg | distinct_count | INFO | 30 |
| dep_code | not_null | PASS | 100.00% |
| dep_code | distinct_count | INFO | 327 |
| dep_name | distinct_count | INFO | 382 |
| brand | distinct_count | INFO | 852 |
| model | distinct_count | INFO | 4169 |
| vin | distinct_count | INFO | 0 |
| make_year | not_null | PASS | 100.00% |
| make_year | range | PASS | [1900, 2027] |
| make_year | distinct_count | INFO | 75 |
| color | not_null | PASS | 100.00% |
| color | distinct_count | INFO | 11 |
| kind | not_null | PASS | 100.00% |
| kind | distinct_count | INFO | 12 |
| body | distinct_count | INFO | 48 |
| purpose | not_null | PASS | 100.00% |
| purpose | distinct_count | INFO | 3 |
| fuel | distinct_count | INFO | 4 |
| capacity | range | FAIL | [0, 20000] |
| capacity | distinct_count | INFO | 1654 |
| own_weight | range | FAIL | [40, 45000] |
| own_weight | distinct_count | INFO | 3836 |
| total_weight | range | FAIL | [60, 90000] |
| total_weight | distinct_count | INFO | 2828 |
| n_reg_new | distinct_count | INFO | 178883 |
| payload | range | FAIL | [0, 35000] |
| payload | distinct_count | INFO | 4577 |
| secondary_fuel | distinct_count | INFO | 2 |
| n_reg_latin | distinct_count | INFO | 178825 |
| is_valid_plate | not_null | PASS | 100.00% |
| raw_vin | distinct_count | INFO | 0 |
| _table_ | row_count | PASS |  |